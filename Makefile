...
VPATH=test sample src PDPKS ECDSA
...
$(EXE_DIR)/PDPKS.exe: $(OBJ_DIR)/PDPKS_test.o $(BN512_LIB) $(MCL_LIB)
	$(PRE)$(CXX) $< -o $@ $(BN512_LIB) $(MCL_LIB) $(LDFLAGS) -lstdc++
...
$(EXE_DIR)/ECDSA.exe: $(OBJ_DIR)/ecdsa.o $(ECDSA_LIB) $(MCL_LIB) src/ecdsa_c.cpp include/mcl/ecdsa.hpp include/mcl/ecdsa.h
...
PDPKS: $(EXE_DIR)/PDPKS.exe $(MCL_LIB)

ECDSA: $(EXE_DIR)/ECDSA.exe $(MCL_LIB)
